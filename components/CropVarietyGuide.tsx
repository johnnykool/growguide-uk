  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-8 sm:py-12">
      ...
      <nav aria-label="Breadcrumb" className="flex flex-wrap items-center gap-x-3 text-sm">
        ...
        <span aria-hidden="true">/</span><span aria-current="page">{guide.name}</span>
      </nav>

      <div className={`mt-7 grid items-center gap-8 md:gap-12 ${guide.image ? "md:grid-cols-[1.5fr_1fr]" : ""}`}>
        <div>
          <h1 className="max-w-2xl font-serif text-4xl leading-tight sm:text-5xl">{guide.heading}</h1>
          <p className="mt-5 max-w-2xl text-lg text-earth-ink">{guide.intro[0]}</p>
          <p className="mt-3 max-w-2xl text-earth-ink">{guide.intro[1]}</p>
          <a href="#comparison" className="variety-text-link mt-4">Jump to the quick comparison <span aria-hidden="true">↓</span></a>
        </div>
        {guide.image && <div className="relative h-52 overflow-hidden rounded-card sm:h-64 md:h-80">
          <Image src={guide.image.src} alt={guide.image.alt} fill priority sizes="(max-width: 768px) 100vw, 430px" className="object-cover" />
        </div>}
      </div>
        ...
        <div className="mt-6 grid gap-5 md:grid-cols-2">
          {guide.varieties.map(v => (
            <article key={v.id} id={v.id} aria-labelledby={`${v.id}-title`} className="flex scroll-mt-24 flex-col rounded-card border border-light-sage bg-sage/10 p-5 sm:p-7">
              ...
              <div className="mt-auto pt-6"><SeedLink crop={guide.slug} variety={v.id} name={v.name} retailer={v.retailer} /></div>
            </article>
          ))}
        </div>
      ...
      <section id="comparison" aria-labelledby="comparison-title" className="mt-12 scroll-mt-24 sm:mt-16">
        ...
        <p className="mt-2 text-earth-ink">{guide.comparisonIntro}</p>
        <div role="region" aria-label={`Comparison of ${guide.name.toLowerCase()}, scroll horizontally on small screens`} tabIndex={0} className="mt-5 overflow-x-auto rounded-btn border border-light-sage focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-dark-earth focus-visible:ring-offset-2 focus-visible:ring-offset-cream">
          <table className="w-full min-w-[640px] text-left text-sm text-earth-ink">
            <caption className="sr-only">{guide.name} compared by best use, growing place and mature size</caption>
            ...
            <tbody>{guide.varieties.map(v => <tr key={v.id} className="border-t border-light-sage/60">
      ...
      <section aria-labelledby="timing-title" className="mt-12 grid gap-6 border-y border-light-sage py-8 md:grid-cols-[1fr_2fr]">
        ...
        <div className="max-w-2xl space-y-3 text-earth-ink">
          {guide.timing.map(paragraph => <p key={paragraph}>{paragraph}</p>)}
        </div>
      </section>

      <section aria-labelledby="healthy-crops" className="mt-10 max-w-3xl text-earth-ink">
        <h2 id="healthy-crops" className="font-serif text-3xl">Keeping {guide.name.toLowerCase()} healthy</h2>
        {guide.health.map(paragraph => <p key={paragraph} className="mt-3">{paragraph}</p>)}
      </section>
